...
export const GenexusInsights = () => {
  ...
  const genexusInsights = getInsightsByPartner('genexus');
  const staticFeaturedInsight = genexusInsights.find(insight => insight.featured);

  // Cargar noticias de Genexus desde la base de datos
  useEffect(() => {
    const fetchGenexusNoticias = async () => {
      try {
        ...
        if (response.data.success && Array.isArray(response.data.data)) {
          // Filtrar noticias de categoría Genexus
          const noticiasGenexus = response.data.data.filter((noticia: Noticia) => 
            noticia.categoria === 'Genexus'
          );
          
          // Ordenar por fecha de creación (más reciente primero)
          const noticiasOrdenadas = noticiasGenexus.sort((a: Noticia, b: Noticia) => 
        ...
        console.error('Error cargando noticias de Genexus:', error);
      ...
      <section className="py-16 bg-gradient-to-br from-slate-50 to-blue-50/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <Badge variant="secondary" className="mb-6 bg-red-50 text-red-700 border-red-200 font-medium">
              🚀 GeneXus Insights
            </Badge>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              Últimas novedades <span className="text-[#2e4bce]">GeneXus</span>
            </h2>
            <p className="text-xl text-gray-700 max-w-3xl mx-auto mb-8">
              Pronto publicaremos las últimas tendencias, casos de éxito y mejores prácticas en desarrollo low-code con GeneXus.
            </p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-700 max-w-3xl mx-auto">
            Mantente al día con las últimas innovaciones en desarrollo low-code y automatización de aplicaciones.
          </p>
        </div>
