...
export const GoogleInsights = () => {
  ...
  const googleInsights = getInsightsByPartner('google');
  const staticFeaturedInsight = googleInsights.find(insight => insight.featured);

  // Cargar noticias de Google desde la base de datos
  useEffect(() => {
    const fetchGoogleNoticias = async () => {
      try {
        ...
        if (response.data.success && Array.isArray(response.data.data)) {
          // Filtrar noticias de categoría Google
          const noticiasGoogle = response.data.data.filter((noticia: Noticia) => 
            noticia.categoria === 'Google'
          );
          
          // Ordenar por fecha de creación (más reciente primero)
          const noticiasOrdenadas = noticiasGoogle.sort((a: Noticia, b: Noticia) => 
        ...
        console.error('Error cargando noticias de Google:', error);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              Últimas novedades <span className="text-[#2e4bce]">Google</span>
            </h2>
            <p className="text-xl text-gray-700 max-w-3xl mx-auto mb-8">
              Pronto publicaremos las últimas tendencias, casos de éxito y mejores prácticas en tecnologías Google.
            </p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-700 max-w-3xl mx-auto">
            Mantente al día con las últimas innovaciones en Google Cloud, AI/ML y herramientas de productividad.
          </p>
        </div>
