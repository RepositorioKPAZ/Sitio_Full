    ...
    const fetchNoticias = async () => {
      try {
        ...
        if (response.data.success && Array.isArray(response.data.data)) {
          ...
          const noticiasDestacadas = todasLasNoticias.filter((noticia: Noticia) => noticia.esDestacada);
          
          if (noticiasDestacadas.length > 0) {
            const ultimaDestacada = noticiasDestacadas.sort((a: Noticia, b: Noticia) => 
          ...
          if (noticiasDestacadas.length > 0) {
            const idDestacada = noticiasDestacadas.sort((a: Noticia, b: Noticia) => 
              new Date(b.created_at).getTime() - new Date(a.created_at).getTime()
            )[0].id;
            
            noticiasParaGrilla = todasLasNoticias.filter((noticia: Noticia) => noticia.id !== idDestacada);
          }
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            ...
            {loading ? (
              ...
              <div className="grid md:grid-cols-3 gap-8 mb-12">
                ...
                {regularNoticias.length < 9 && getFallbackInsights().map((insight, index) => (
                  <ArticleCard
                    key={`insight-${index}`}
                    title={insight.title}
